...
// .toUpperCase()
const tag = tags(['user'.toLowerCase().replace('user'.charAt(0),'user'.charAt(0).toUpperCase())])
  ...
  static async deleteMany(ctx) {
    ...
    if(params.jsonStr !== undefined){
      try {
        ...
        let result = await dbClient.remove('user',paramsData)
        ctx.body = result
...
// 查
  ...
  static async getAll(ctx) {
    ...
    let result = params['page'] && params['pageSize'] ? await dbClient.find('user', paramsData, filterConditions,  params.page, params.pageSize) : await dbClient.find('user', paramsData, filterConditions)
    ctx.body = result
  }
}
