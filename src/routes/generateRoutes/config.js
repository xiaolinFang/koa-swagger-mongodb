...
export default class config {
  ...
  static async deleteMany(ctx) {
    ...
    if (params.jsonStr !== undefined) {
      try {
        ...
        const result = await dbClient.remove('config', paramsData);
        ctx.body = result;
      } catch (e) {
  ...
  static async getAll(ctx) {
    ...
    const result = params.page && params.pageSize ? await dbClient.find('config', paramsData, filterConditions, params.page, params.pageSize) : await dbClient.find('config', paramsData, filterConditions);
    ctx.body = result;
  }
}
